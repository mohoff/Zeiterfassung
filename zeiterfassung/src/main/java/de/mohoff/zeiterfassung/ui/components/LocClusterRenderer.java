 ...
 */
public class LocClusterRenderer<T extends ClusterItem> implements ClusterRenderer<T> {
    ...
    public LocClusterRenderer(Context context, GoogleMap map, ClusterManager<T> clusterManager) {
        mMap = map;
    ...
    private LayerDrawable makeClusterBackground() {
        ...
        return background;*/
        mColoredCircleBackground = new ShapeDrawable(new OvalShape());
        mColoredCircleBackground.getPaint().setColor(0xEE025167);
    ...
    protected boolean shouldRenderAsCluster(Cluster<T> cluster) {
        return (cluster.getSize() > MIN_CLUSTER_SIZE);// My TODO: && (mMap.getCameraPosition().zoom != mMap.getMaxZoomLevel());
    }
    ...
    private class RenderTask implements Runnable {
        ...
        @SuppressLint("NewApi")
        public void run() {
            if (clusters.equals(LocClusterRenderer.this.mClusters)) {
                mCallback.run();
                return;
            }
            ...
            if (LocClusterRenderer.this.mClusters != null && SHOULD_ANIMATE) {
                existingClustersOnScreen = new ArrayList<Point>();
                for (Cluster<T> c : LocClusterRenderer.this.mClusters) {
            ...
            mMarkers = newMarkers;
            LocClusterRenderer.this.mClusters = clusters;
            mZoom = zoom;
